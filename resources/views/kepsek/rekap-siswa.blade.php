        ...
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-header">
                    ...
                    <form method="GET" action="{{ route('kepsek.rekap-siswa') }}" class="d-flex flex-wrap align-items-center gap-2">
                        ...
                        <select name="kelas_id" id="kelas_id" class="form-select form-select-sm" onchange="this.form.submit()">
                            ...
                            @foreach($kelasList as $kelas)
                                <option value="{{ $kelas->id }}" {{ $kelasTerpilih && $kelasTerpilih->id == $kelas->id ? 'selected' : '' }}>
                                    {{ $kelas->nama_kelas }}
                                </option>
                            @endforeach
                        </select>
                                    ...
                                    <tr>
                                        ...
                                        @for ($i = 1; $i <= $jumlahHari; $i++)
                                            @php
                                                ...
                                                $status = $absensi[$siswa->id][$tanggal] ?? '';
                                                $warna = match($status) {
                                                    'Hadir' => 'success',
                                                    'Alpha' => 'danger',
                                                    'Izin'  => 'warning',
